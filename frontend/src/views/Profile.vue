...
export default {
  ...
  setup() {
    ...
    const syncFiles = async () => {
      ...
      try {
        const baseUrl = getBaseUrl()
        const res = await fetch(`${baseUrl}/admin/file-sync`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${currentUser.value.id}`
        ...
        if (res.ok) {
          const data = await res.json()
          refreshMessage.value = data.message || '文件同步完成: ' + 
            (data.details ? `新增 ${data.details.added} 个, 删除 ${data.details.deleted} 个` : '')
        } else {
          const errorData = await res.json()
          refreshMessage.value = errorData.error || '同步失败'
        }
      } catch (error) {
