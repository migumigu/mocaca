...
def scan_media_folder():
    ...
    # 获取数据库中的所有文件记录
    db_files = {v.filename: v for v in Video.query.all()}
    current_files = set()
    deleted_count = 0
    added_count = 0
    ...
    for root, dirs, files in os.walk(app.config['MEDIA_FOLDER']):
        for filename in files:
            if filename.endswith(('.mp4', '.mov', '.avi')):
                ...
                # 2. 如果文件在media目录但不在数据库，添加到数据库
                if rel_path not in db_files and is_portrait_video(filepath):
                    video = Video(filename=rel_path, filepath=filepath)
                    db.session.add(video)
                    added_count += 1
    
    # 3. 检查数据库中有但media目录不存在的记录，删除这些无效记录
    for filename, video in db_files.items():
        full_path = os.path.join(app.config['MEDIA_FOLDER'], filename)
...
@app.route('/api/admin/file-sync', methods=['POST'])
def admin_file_sync():
    """管理员手动触发文件同步(需要管理员权限)"""
    # 检查用户权限
